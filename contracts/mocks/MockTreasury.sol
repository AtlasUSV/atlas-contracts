...
contract MockUniversalTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public sUSV;
  uint256 public sUSVQueue; // Delays change to sUSV address
  ...
  constructor(
    ...
    // isLiquidityToken[ _USVDAI ] = true;
    // liquidityTokens.push( _USVDAI );
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sUSV).balanceOf(msg.sender); // Can only borrow against sUSV held
  ...
  /**
        ...
        @param _amount uint
     */
  function repayDebtWithUSV(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");
    ...
    returns (uint256 value_)
  {
    if (isReserveToken[_token]) {
      ...
      value_ = _amount.mul(10**IERC20(USV).decimals()).div(
        10**IERC20(_token).decimals()
      );
    } else if (isLiquidityToken[_token]) {
      value_ = IBarterCalculator(barterCalculator[_token]).valuation(
        _token,
        _amount
      );
    }
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SUSV) {
      // 9

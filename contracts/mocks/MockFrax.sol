...
contract MockFRAX is LibNote {
  ...
  event Transfer(address indexed src, address indexed dst, uint wad);
    ...
    modifier auth {
        require(wards[msg.sender] == 1, "MockFrax/not-authorized");
        _;
    }

    // --- ERC20 Data ---
    string  public constant name     = "MockFRAX TOKEN";
    string  public constant symbol   = "MockFRAX";
    string  public constant version  = "1";
    uint8   public constant decimals = 18;
    uint256 public totalSupply;
    uint public dailyMockFraxLimit;
    ...
    constructor(uint256 chainId_) {
        ...
        dailyMockFraxLimit = 10000000000000000000000;
    }
    ...
    function transferFrom(address src, address dst, uint wad) public returns (bool) {
      ...
      require(balanceOf[src] >= wad, "MockFrax/insufficient-balance");
        if (src != msg.sender && _allowance( src, msg.sender ) != uint(-1)) {
            require(_allowance( src, msg.sender ) >= wad, "MockFrax/insufficient-allowance");
    ...
    function adjustDailyMockFraxLimit(uint _limit) external auth {
        dailyMockFraxLimit = _limit;
    }

    function mint(address usr, uint wad) external {

      if(wards[msg.sender] == 0) {
        require(add(wad, fraxMintedToday[msg.sender]) <= dailyMockFraxLimit || sub(block.number, lastMintRestart[msg.sender]) >= 6500 && wad <= dailyMockFraxLimit, "Over daily MockFrax Limit");
    ...
    function burn(address usr, uint wad) external {
        require(balanceOf[usr] >= wad, "MockFrax/insufficient-balance");
        if (usr != msg.sender && _allowance( usr, msg.sender ) != uint(-1)) {
            require(_allowance( usr, msg.sender ) >= wad, "MockFrax/insufficient-allowance");
                    ...
                    bool allowed, uint8 v, bytes32 r, bytes32 s) external
    {
        ...
        require(holder != address(0), "MockFrax/invalid-address-0");
        require(holder == ecrecover(digest, v, r, s), "MockFrax/invalid-permit");
        require(expiry == 0 || block.timestamp <= expiry, "MockFrax/permit-expired");
        require(nonce == nonces[holder]++, "MockFrax/invalid-nonce");

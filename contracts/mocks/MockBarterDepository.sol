...
contract MockUniversalBarterDepository is Ownable {
  ...
  event BarterRedeemed(
    address indexed recipient,
    uint256 payout,
    uint256 remaining
  );
  event BarterPriceChanged(
    uint256 indexed priceInUSD,
  ...
  uint256 public totalDebt; // total value of outstanding barters; used for pricing
  ...
  struct Terms {
    ...
    uint256 fee; // as % of barter payout, in hundreths. ( 500 = 5% = 0.05 for every 1 paid)
    uint256 maxDebt; // 9 decimal debt ratio, max % total supply created as debt
  }

  // Info for barterer
  struct Barter {
    uint256 payout; // USV remaining to be paid
   ...
   *  @param _input uint
   */
  function setBarterTerms(PARAMETER _parameter, uint256 _input)
    external
    onlyPolicy
  {
  ...
  function deposit(
    ...
    uint256 priceInUSD = barterPriceInUSD(); // Stored in barter info
    uint256 nativePrice = _barterPrice();

    require(_maxPrice >= nativePrice, "Slippage limit: more than max price"); // slippage protection

    uint256 value = ITreasury(treasury).valueOfToken(principle, _amount);
    uint256 payout = payoutFor(value); // payout to barterer is computed
    ...
    if (fee != 0) {
      // fee is transferred to AtlasTeam
   ...
   *  @return price_ uint
   */
  function barterPrice() public view returns (uint256 price_) {
   ...
   *  @return price_ uint
   */
  function _barterPrice() internal returns (uint256 price_) {
    price_ = terms.controlVariable.mul(debtRatio()).add(1000000000).div(1e7);
    if (price_ < terms.minimumPrice) {
      price_ = terms.minimumPrice;
   ...
   *  @return price_ uint
   */
  function barterPriceInUSD() public view returns (uint256 price_) {
    if (isLiquidityBarter) {
      price_ = barterPrice()
        .mul(IBarterCalculator(barterCalculator).markdown(principle))
        .div(100);
    } else {
  ...
  function debtRatio() public view returns (uint256 debtRatio_) {
    uint256 supply = IERC20(USV).totalSupply();
    ...
    returns (uint256 percentVested_)
  {
    ...
    uint256 blocksSinceLast = block.number.sub(barter.lastBlock);
    uint256 vesting = barter.vesting;
    ...
    returns (uint256 pendingPayout_)
  {
    ...
    uint256 payout = barterInfo[_depositor].payout;
